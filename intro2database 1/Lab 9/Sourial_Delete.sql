...
DELIMITER $$
CREATE TRIGGER lasttrigger 
BEFORE DELETE ON professor 
FOR EACH ROW
BEGIN 
	INSERT INTO professor_audit_trail (id, ProfessorName, ProfessorProgram)
    values (old.ProfessorId,old.ProfessorName,old.ProfessorProgram);
END$$
DELIMITER ;
...
DROP INDEX StudentProgram_idx
ON student;

DROP TRIGGER lasttrigger;
